...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      ...
      <span
        aria-hidden
        className="scroll-progress absolute inset-x-0 top-0 z-10 h-0.5 origin-left bg-gradient-to-r from-electric via-violet to-cyan"
      />
      ...
      {/* Mobile drawer (slides in from the right). Stays mounted so it can
          transition both in and out purely with CSS. */}
      <div className="md:hidden" aria-hidden={!open}>
        ...
        <aside
          ...
          className={`fixed inset-y-0 right-0 z-50 flex w-[82%] max-w-sm flex-col border-l border-navy/10 bg-ink/95 shadow-[-24px_0_60px_-24px_rgba(0,0,0,0.35)] backdrop-blur-2xl transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] ${
            ...
            <ul className="flex flex-col gap-1.5">
              {LINKS.map((l, i) => {
                ...
                return (
                  <li
                    key={l.href}
                    className={open ? "anim-drawer-link" : undefined}
                    style={open ? { animationDelay: `${0.08 + i * 0.05}s` } : undefined}
                  >
                    <Link
                      href={l.href}
                      tabIndex={open ? 0 : -1}
